#!/usr/bin/env bash
# Regenerate the PHP wire layer (src/Wire) with jane-php.
#
# NOT the generator any more: beak emits src/Wire in-process Go
# (tools/beak/gens/sdkphp.go). This is kept as the parity ORACLE the port is
# checked against — tools/beak:sdk-php-parity runs it into a temp tree and
# compares. Editing it changes what the port is measured against, nothing that
# ships.
#
...
# The parity check points BEAK_STAGE at a temp tree, so the comparison never
# touches the committed one. A direct invocation regenerates in place.
...
# Skips cleanly without the PHP toolchain. The parity check treats an empty
# result as a failure rather than a pass, so a silent skip cannot read as
# agreement.
if ! command -v php >/dev/null 2>&1 || [ ! -x "$here/vendor/bin/jane-openapi" ]; then
    echo "==> php/jane toolchain absent — nothing to compare against (src/Wire is generated by beak, not by this script)"
    exit 0
fi
